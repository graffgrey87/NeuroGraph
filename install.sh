#!/bin/bash
# NeuroGraph Installer v8.0 (Native Injection)

WORKSPACE="/workspace"
LORA_PATH="/workspace/ComfyUI/models/loras/Qwen_Pack"
WC_PATH="/workspace/ComfyUI/wildcards"

echo "🎨 [NeuroGraph] Native Installer запустился..."
...
# 2. Файлы бота (копируем из папки, куда склонировал RunPod)
# Внимание: при этом методе файлы будут лежать в /workspace/installer
...
# 3. Функция скачивания (Исправленная, без кавычек)
download_model() {
    local url=$1
    local dest=$2
...
echo "✅ [NeuroGraph] Установка завершена! Возвращаю управление системе..."
