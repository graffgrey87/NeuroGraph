...
## 17. Группировка референсов в чате (2026-02-27)

- **Отложенная обработка (Debounce):** Добавлен механизм временных задержек в обработчик изображений (`bot.py`). Теперь, когда пользователь отправляет альбом (MediaGroup) с несколькими референсами картинками, бот собирает их в единый список и, спустя небольшую паузу, отвечает одним аккуратным сообщением, содержащим количество загруженных фото и имя последнего. Это полностью устраняет проблему замусоривания чата.
