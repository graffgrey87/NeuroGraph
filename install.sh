#!/bin/bash
# NeuroGraph Installer v9.0 (Post-Start Mode)

WORKSPACE="/workspace"
LORA_PATH="/workspace/ComfyUI/models/loras/Qwen_Pack"
WC_PATH="/workspace/ComfyUI/wildcards"

# Даем системе продышаться 10 секунд после старта ComfyUI
sleep 10

echo "🎨 [NeuroGraph] Система загружена. Начинаю установку..."
...
# 3. Функция скачивания (Исправленная)
download_model() {
    local url=$1
    local dest=$2
    if [ -z "$HF_TOKEN" ]; then
...
# 4. Скачивание (фоном, чтобы не тормозить бот)
